...
class AreAllSampleMeasured(Behaviour):
    def __init__(self, name: str):
        ...
        self.bb.register_key('next_sample_index', access=Access.WRITE)
...
class SetNextSample(Behaviour):
    ...
    def update(self) -> Status:
        self.logger.debug(f'Update {self.name} node.')
        if self.bb.next_sample_index is None:
...
def create_root():
    ...
    topics_to_bb.add_children([gantry_to_bb, analytical_to_bb, camera_to_bb])
...
def main():
    ...
    blackboard.next_sample_index = None
